...
public class ServidorController implements Serializable {
    ...
    public void fillServidorList() {
        ...
        servidores.removeIf(serv -> loginController.getServidorAutenticado().equals(serv));
    ...
    public void cadastrarServidor() {
        try {
            Servidor tmp = servidorDAO.buscarPorEmail(servidor.getEmail().trim());
            if (tmp != null) {
                Util.addMessageError("Erro ao cadastrar servidor. Email em uso.");
                return;
            }

            tmp = servidorDAO.buscarPorProntuario(servidor.getProntuario());
            if (tmp != null) {
                Util.addMessageError("Erro ao cadastrar servidor. Prontuário em uso.");
                return;
            }

            servidor.setSenha(HashUtils.gerarHash(servidor.getSenha()));
    ...
    public void editarServidor() {
        ...
        Servidor tmp = servidorDAO.buscarPorEmail(servidor.getEmail().trim());
        if (tmp != null && !tmp.getProntuario().equals(servidor.getProntuario())) {
            Util.addMessageError("Erro ao editar servidor. Email em uso.");
            return;
        }
        try {
            if (servidor.getSenha().equals("")) {
                Servidor s = servidorDAO.buscarPorProntuario(servidor.getProntuario());
                s.setEmail(servidor.getEmail());
                s.setTipo(servidor.getTipo());
                s.setNome(servidor.getNome());
                s.setPdi(servidor.getPdi());
